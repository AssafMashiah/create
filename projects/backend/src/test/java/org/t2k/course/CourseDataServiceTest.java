...
import org.t2k.cgs.model.course.CourseDifferentiation;
...
import org.t2k.cgs.scheduling.CleanupScheduler;
...
public class CourseDataServiceTest extends AbstractTestNGSpringContextTests {
...
//        int publisherId = Integer.parseInt(cgsData.get("publisherId").toString());
    ...
    public void gettingTocItemsDataForImportLessonGetsOnlyNecessaryData() throws DsException, DaoException, IOException {
        ...
        try {    // insert data into DB
            ...
            Assert.assertEquals(((DBObject) tocItemsContents.get(0)).get("format"), "EBOOK");
    ...
    @Test
    public void getTocItemsBasicDataForCourseLessonsMenuReturnsOnlyNeededLessons() throws DsException, DaoException, IOException {

        String courseId = "courseIdForDeleteTest";
        int publisherId = 100101;
        ...
        String course = " {\n" +
                ...
                "            \"tocItemRefs\" : [{\"cid\":\"2dc39b39-bee3-4fc8-a2b3-5e756770c12a\",\n" +
                "\t\t\t\"type\":\"assessment\"\n" +
                "\t\t\t},{\"cid\":\"2dc39b39-bee3-4fc8-a2b3-5e756770c12b\",\n" +
                "\t\t\t\"type\":\"assessment\"\n" +
                "\t\t\t},{\"cid\":\"b8fecc80-0d90-44a0-80cc-a98919173142\",\n" +
                "\t\t\t\"type\":\"lesson\"\n" +
                "\t\t\t},{\"cid\":\"b8fecc80-0d90-44a0-80cc-a98919173143\",\n" +
                "\t\t\t\"type\":\"lesson\"\n" +
                "\t\t\t}],\n" +
                "            \"tocItems\" : []\n" +
                "        },\n" +
        ...
        String assessmentDeleted = "{\n" +
                ...
                "        },\n" +
                "        \"schema\" : \"course_v4\",\n" +
                "        \"title\" : \"assessmentDeleted for test\",\n" +
                "        \"type\" : \"assessment\",\n" +
                "        \"useForDifferentialRecommendation\" : false,\n" +
                "        \"containsInstructionalSequence\" : false,\n" +
                "        \"checkingType\" : \"auto\"\n" +
        ...
        String lessonDeleted = "{\n" +
                ...
                "        \"standards\" : [],\n" +
                "        \"standardPackages\" : [],\n" +
                "        \"title\" : \"lessonDeleted for test\",\n" +
                "        \"type\" : \"lesson\"\n" +
                "}";

        TocItemCGSObject normalLessonObject = new TocItemCGSObject(lesson, publisherId, courseId, EntityType.LESSON);
        TocItemCGSObject normalDeletedLessonObject = new TocItemCGSObject(lessonDeleted, publisherId, courseId, EntityType.LESSON);
        TocItemCGSObject normalAssessmentObject = new TocItemCGSObject(assessment, publisherId, courseId, EntityType.ASSESSMENT);
        TocItemCGSObject normalDeletedAssessmentObject = new TocItemCGSObject(assessmentDeleted, publisherId, courseId, EntityType.ASSESSMENT);
        CourseCGSObject courseCGSObject = new CourseCGSObject(course, publisherId);
        LockUser lockUser = new LockUser("mockUser", "mockUser", "mockUser", "mockUser", publisherId);

        try {    // insert data into DB
            tocItemsManager.save(normalLessonObject, lockUser);       // add a normal lesson
            tocItemsManager.save(normalDeletedLessonObject, lockUser);       // add a deleted lesson
            tocItemsManager.save(normalAssessmentObject, lockUser);   // add an assessment
            tocItemsManager.save(normalDeletedAssessmentObject, lockUser);   // add a deleted assessment
            coursesDao.saveCourseCGSObject(courseCGSObject);

            tocItemsManager.delete(normalDeletedLessonObject);       // delete lesson
            tocItemsManager.delete(normalDeletedAssessmentObject);       // delete assessment

            // check the api for getting only the relevant tocItems for the course
            List<TocItemCGSObject> result1 = courseDataService.getTocItemsBasicDataForCourseLessonsMenu(publisherId, courseId);

            // assert second tocItem is our lesson
            Assert.assertEquals(result1.size(), 2);
            Assert.assertEquals(result1.get(0).getTitle(), normalLessonObject.getTitle());
            Assert.assertEquals(result1.get(0).getContentId(), normalLessonObject.getContentId());
            Assert.assertEquals(result1.get(1).getTitle(), normalAssessmentObject.getTitle());
            Assert.assertEquals(result1.get(1).getContentId(), normalAssessmentObject.getContentId());
