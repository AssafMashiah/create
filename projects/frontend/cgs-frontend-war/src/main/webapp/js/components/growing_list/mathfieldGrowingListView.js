define(['jquery', './growingListComponentView', 'mustache', 'localeModel', 'components/mathfield/MathField',
    'text!components/growing_list/html/growingList.html',
    'text!components/growing_list/html/mathfieldListRow.html'],
    function f1436($, growingListComponentView, mustache, localeModel, MathFieldView, template, row_template) {

        var mathfieldGrowingListView = growingListComponentView.extend({

            initialize: function f1437(options) {
                this._super(options);
            },

            /*init default data for list render*/
            initData: function f1438(options) {
                ...
                this.rowTemplate = row_template;
            },

            render: function f1439() {
                this._super();
                //init all data ad mathfields components
                _.each(this.data, function f1440(mathfieldItem, index) {
                    var mathfieldContainer = this.$(".mathfieldContainer")[index],
                        container = this.$(".content.column")[index];
                    this.initMathfield(mathfieldContainer, container, mathfieldItem.markup);

                }, this);

            },
            endMathfieldEdit: function () {
                //mathfield workaround for dispose the editor mathfields
               this.selectedMathfield && this.selectedMathfield.mathField && this.selectedMathfield.mathField.view && this.selectedMathfield.mathField.view.endEdit();

            },
            setDefaultMarkup: function (markup) {
                this.defaultMarkup = markup;
            },
            setMathfieldState: function (state) {
                //init all data ad mathfields components

                _.each(this.mathfieldArray, function f1440(mathfieldItem, index) {
                    if (mathfieldItem && mathfieldItem.mathField && (mathfieldItem.mathField.getWidthEM() * 16) > 218) {
                        this.$el.find('.mathfieldContainer').css('overflow-x', 'scroll');
                    }
                    mathfieldItem.mathField.view.cfg.data.empty();
                    mathfieldItem.mathField.setState(state);
                    mathfieldItem.mathField.view.setEnabled(true);
                }, this);
             ...
             */
            serialize_list_data: function f1441() {
                return require('cgsUtil').cloneObject(this.data);
            },

            /*override add new row function*/
            add_new: function f1442(event) {
                this._super(event);

                var container = this.$(".content.column:last"),
                    mathfieldContainer = this.$("div.js-growing-row:last").find(".mathfieldContainer");
                this.initMathfield(mathfieldContainer, container, this.defaultMarkup);
            },

            /*override remove row function*/
            remove_row: function f1443(event) {
                var indexToRemove = $(event.currentTarget).closest('div.js-growing-row').find('.index').text();
                var removedData = this.data.splice((parseInt(indexToRemove) - 1), 1);
                var removedMathfield = this.mathfieldArray.splice((parseInt(indexToRemove) - 1), 1);
                this.mathfieldArrayRef = _.omit(this.mathfieldArrayRef, function(item) {
                    return item == removedMathfield[0];
                });
                _.invoke(removedMathfield, 'dispose');
                this._super(event);
            },

            updateData: function f1444() {
                this.data = [];
                _.each(this.mathfieldArray, function f1445(item, index) {
                    var id = this.$('div.js-growing-row').eq(index).data('item-id');
                    if (!id) {
                        id = require('repo').genId();
                        this.$('div.js-growing-row').eq(index).attr('data-item-id', id);
                        this.$('div.js-growing-row').eq(index).data('item-id', id);
                    }
                    this.data.push({
                        id: id,
                        markup: item.mathField.view.getMarkUpValue(),
                        value: item.mathField.view.getValue(),
                        index: index + 1,
                        widthEM: item.mathField.view.getWidthEM()
                    });
                }, this);
                this.updateModel();
            ...
            setKeyboardEvent: function f1446() {
                $(document).one("mousedown.growing_list", function f1447(e) {
                    this.selectedMathfield && this.selectedMathfield.mathField.view.endEdit();
                    //$(document).trigger("mouseup.endMathfieldEdit");
                }.bind(this))
            },
            /*initializ mathfield component */
            initMathfield: function f1448(parent, container, markup) {

                var attributes = require('cgsUtil').getRepoDefaultData('mathfield');
                attributes.maxHeight = this.options.maxHeight || 'secondLevel'; // ovverride default value (second level)
                ...
                var maxWidth = (_.max(_.pluck(this.options.data, 'widthEM')) * 16);

                var iframeParent = $(parent);
                var index = (this.mathfieldArray && this.mathfieldArray.length) || 0;
                var mathfieldContainerId = 'mathfield_container_' + index;
                ...
                iframeParent.css('overflow-x', maxWidth > 218 ? 'scroll' : 'hidden');
                ...
                var initHTML = '<!DOCTYPE html>';
                    ...
                    initHTML += '<div class="mf_container" id="' + mathfieldContainerId +'"></div>';
                    initHTML += '</body>';
                    initHTML += '</html>';
                ...
                this.mathfieldArrayRef = this.mathfieldArrayRef || {};

                this.defer = _.defer(function() {
                    var config = {
                        data: mfData,
                        editMode: (this.options.propsView.answerMathfield || this.defaultMarkup) ? 'completion' : 'on',
                        parent: $(iframeDoc).find('#' + mathfieldContainerId),
                        container: $(parent).find('.mathfieldContainer'),
                        iframeDoc: $(iframeDoc),
                        onChange: function () {
                            if (mathfield && mathfield.mathField && (mathfield.mathField.getWidthEM() * 16) > 218) {
                                iframeParent.css('overflow-x', 'scroll');
                            }
                        },
                        onStartEdit: function f1449() {
                            if (!this.selectedMathfield) {
                                this.selectedMathfield = this.mathfieldArrayRef[mathfieldContainerId];
                                this.setKeyboardEvent();

                            } else {
                                this.selectedMathfield && this.selectedMathfield.view && this.selectedMathfield.view.keyboard.dispose()
                            }
                           ...
                           this.selectedMathfield && this.selectedMathfield.view && this.selectedMathfield.view._input.off('focus').one('focus', function () {
                                ...
                                _.each( this.mathfieldArrayRef , function (mf, mfId) {
                                    if (mfId !== mathfieldContainerId) {
                                        mf.mathField.view.endEdit();
                                    } 
                                }, this);
                            }.bind(this)); 
                        }.bind(this),
                        onEndEdit:  _.debounce(this.updateData.bind(this), 500, {
                              'leading': true,
                              'trailing': false
                            })
                    };
                    ...
                    var mathfield = new MathFieldView(config);
                    ...
                    if (mathfield.mathField) {
                        ...
                        this.mathfieldArray.push(mathfield);
                        this.mathfieldArrayRef[mathfieldContainerId] = mathfield;
            ...
            // fits the index of the row (gui & data)
            fix_index: function f1450() {
                this._super();
                _.each(this.data, function f1451(item, index) {
                    item.index = index + 1;
                });
            },
            dispose: function f1452() {
                ...
                delete this.mathfieldArrayRef;
                delete this.defaultMarkup;
            }
        });
        return mathfieldGrowingListView;
    });