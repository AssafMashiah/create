define(['jquery', 'BaseStageContentView', 'rivets', 'mustache', 'events','repo','assets', 'localeModel', 'translate',
        'components/mathfield/MathField', 'mathjax', 'text!modules/TextViewerEditor/templates/TextViewerSmallStagePreview.html'
    ],
function($, BaseStageContentView, rivets, Mustache, events, repo, assets, localeModel,i18n, MathFieldView, mathjax, template) {

	var TextViewerSmallStageView = BaseStageContentView.extend({
		...
		//ovverride parent function, because we dont want to render the children
		...
		setIframe: function(iframe){
            ...
            var initHTML = '<!DOCTYPE html>';
				...
				initHTML +=		'</head>';
				initHTML +=		'<body spellcheck="' + require('userModel').getAccount().enableFullSpellChecker + '" class="texteditor smallStage">';
				initHTML += this.controller.record.data.title;
				initHTML +=		'</body>';
				initHTML +=	'</html>';

            // inject html to the iframe
            this.document.open();
            this.document.write(initHTML);
            this.document.close();
            ...
            if (this.controller.record.data["data-placeholder"]) {
            	this.body.attr('data-placeholder', i18n.tran('click to edit definition'));
            }

            // disable text select on small stage
            // if (this.controller.record.data.stageReadOnlyMode)
				this.body.addClass('unselect');

			//set the data from repo, and add class of style
			this.body.addClass(this.controller.record.data.textEditorStyle);


			if (this.controller.record.data.styleOverride) {
				$('div', this.body).attr('class', 'cgs ' + this.controller.record.data.styleOverride).attr('customStyle', this.controller.record.data.styleOverride);
			}

			//replace component tag with real img tag.
			_.each($(this.body).find('component'), _.bind(function(component){
				var obj= repo.get(component.id);
				var _img = $("<img />");

				var _attributes = {
					"inlineImage": function f1142() {
						return {
							"class": 'component',
							"height": '20px',
				...
				_img.attr('type', obj.type);

				if (_attributes[obj.type]) {
					_img.attr(_attributes[obj.type]());
				} else {
					_img.attr(_attributes['default']());
				}

				_img.load(function f1145() {
					this.setIframeHeight();
				}.bind(this))

				$(component).replaceWith(_img);
			}, this));
			...
			this.defer = _.defer(function() {
                ...
                iframeStyle.addEventListener( "load" , function(){
					//init mathfield component in edit mode off, replace mathfield tag
					_.each($(this.body).find('mathfield'), _.bind(function(mathfield){
						var markup= this.controller.record.data.mathfieldArray[mathfield.id].markup;
                        ...
                        mathfieldtag.attr({
                             id: mathFieldId,
                             'class': "mathfieldItem mathFieldWrapper"
                        });
                ...
                this.document.head.appendChild( iframeStyle );
				 ...
				 _.delay( this.setIframeHeight.bind(this), 200);
			}.bind(this));

			//in this point iframe css is empty
			...
			this.body.find('[contenteditable=true]').attr('contenteditable', false)
			this.setPeregraphFloatDirection();

		},

		setPeregraphFloatDirection: function f1146() {
			var direction = localeModel.getConfig('direction');
			var floatDirection = {
				"rtl": "right",
				"ltr": "left"
			};

			this.body.find('div.cgs').css('float', floatDirection[direction]);
			this.body.find('div.cgs').css('direction', direction);
			this.body.find('div.cgs').css('margin', direction == "ltr" ? "0px 10px 0px 0px" : "0px 0px 0px 10px" );
		},
		//decrease is for remove from the scroll height the last peregraph deleted
		setIframeHeight: function f1147(decrease) {
			...
			if (this.body.children().length > 0) {
				this.body.children().each(function() {
					...
					_totalHeight += _item.outerHeight(true);
				});

				_totalHeight += _.isNumber(parseInt(this.body.css('margin-top'))) ? parseInt(this.body.css('margin-top')) : 0;
				_totalHeight += _.isNumber(parseInt(this.body.css('margin-bottom'))) ? parseInt(this.body.css('margin-bottom')) : 0;
			} else {
				_totalHeight = this.body.outerHeight(true);
		...
		resetDocumentProperties: function f1148() {
		...
		setDocument: function f1149() {
			this.document = this.iframe.contents().get(0);
		},
		setHead: function f1150() {
			this.head = this.iframe.contents().find('head');
		},
		setBody: function f1151() {
			this.body = this.iframe.contents().find('body');
		},
		//at small stage, double click to the iframe
		bubbleUpWrapperEvents: function(){

			var $wrapper = this.iframe.parents('.element_preview_wrapper').eq(0);

			this.body.bind('dblclick', function(){
		...
		initMathField:function(mathFieldId, attributes, markup){
			...
			var mf = new MathFieldView({
				data: mfData,
				parent: $(this.document).find("#"+mathFieldId),
				container: this.body,
		...
		dispose: function() {
            ...
            type: 'TextViewerSmallStageView'
        });

	return TextViewerSmallStageView;
