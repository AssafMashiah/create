...
function (_, Mustache, rest, files, restDictionary, showMessage, busy, migrationUtil) {
	...
	var Dao = (function () {
		...
		function onServerError(callback, continueCallback, jqXHR, textStatus, errorThrown) {
			...
			else if (_.isFunction(callback)) { //check if we pass error handler
			...
			else if (_.isFunction(continueCallback)) {
				continueCallback();
			}
		...
		*/
		function remote(config, callback, error, disableCache) {
			setBusy(true);
			...
			return rest.api(pathData,
					config.data, 
					onRemoteResponse.bind(this, callback), 
					onServerError.bind(this, error, callback),
					disableCache);
		}
		...
		function setLocal(config, callback) {
			files.prepareDirs(config.pathParams.publisherId, config.pathParams.courseId, function () {
				...
				files.saveObject(config.data, fileName, filePath, callback);
			});
		}
		...
		return {
			...
			getRestApiValidConfig: getRestApiValidConfig,
			stop: stop
		};
	})();
